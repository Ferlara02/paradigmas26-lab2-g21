...
/** Responsable de todas las operaciones de entrada/salida: descarga de feeds,
  * lectura de archivos locales y extracción de texto.
  *
...
object FileIO {
  ...
  def readSubscriptions(): List[String] = {
    ...
    (parsed \\ "url").children.collect { case JString(url) =>
      url
    }
  }
  ...
  /** Extrae los títulos de posts del JSON de Reddit.
    *
    * @param json
    *   respuesta JSON de la API de Reddit
    * @return
    *   lista de títulos de posts
    */
  def extractPostTitles(json: String): List[String] = {
    val parsed = parse(json)
    (parsed \\ "title").children.collect { case JString(title) =>
      title
    }
  }

  /** Lee un archivo de texto línea por línea. Ignora líneas vacías y líneas que
    * comienzan con '#' (comentarios).
    *
    * @param filePath
    *   ruta al archivo
    * @return
