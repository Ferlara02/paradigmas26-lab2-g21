...
class Ejercicio3_detectEntities extends munit.FunSuite{
    ...
    test("detectEntities with empty dictionary returns empty list") {
        assertEquals(Analyzer.detectEntities("I love Scala", List.empty), List.empty)
    }
    ...
    test("detectEntities detects a name multiple times but only instantiates it once"){
        val text = "Pepe Pecas pica papas con un pico. Con un pico pica papas Pepe Pecas"
        val dictionary = List(
    ...
    test("detectEntities has a dictionary with repeated entries but only instantiates it once"){
        val text = "Donald Knuth is a Person who deserves more recognition at my University, alongside other remarkable academics."
        val dictionary = List(
    ...
    test("detectEntities doesn't produce false positives from entityType keywords"){
        val text = "Donald Knuth is a Person who deserves more recognition at my University, alongside other remarkable academics."
        val dictionary = List(
        ...
        val resultado = Analyzer.detectEntities(text, dictionary)
        assert(!resultado.exists(e => e.text == "Martin Odersky" && e.entityType == "Person"))
        assert(!resultado.exists(e => e.text == "Ada Lovelace" && e.entityType == "Person"))
    }
    ...
    test("detectEntities return correct entity detection"){
        ...
        assert(resultado.exists(e => e.text == "Scala" && e.entityType == "ProgrammingLanguage"))
        assert(resultado.exists(e => e.text == "EPFL" && e.entityType == "University"))
        assert(resultado.exists(e => e.text == "Martin Odersky" && e.entityType == "Person"))
    ...
    test("detectEntities doesn't includes word segments found in the dictionary"){
