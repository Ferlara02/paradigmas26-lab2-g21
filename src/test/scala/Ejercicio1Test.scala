class Ejercicio1 extends munit.FunSuite {
    ...
    test("returns correct format for each entity type"){
        ...
        assertEquals(entities(0).describe, "[Person] Alan Turing", "Wrong implementation for class \"Person\"")
        assertEquals(entities(1).describe, "[University] MIT", "Wrong implementation for class \"University\"")
        assertEquals(entities(2).describe, "[ProgrammingLanguage] Scala", "Wrong implementation for class \"ProgrammingLanguage\"")
        assertEquals(entities(3).describe, "[Place] San Francisco", "Wrong implementation for class \"Place\"")
    }

    test("University is a subclass of Organization"){
        val mit = new University("MIT")
        assertEquals(mit.entityType, "University")
        assertEquals(mit.isInstanceOf[Organization], true, "\"University\" should be a subclass of \"Organization\"")
    }
    ...
    test("ProgrammingLanguage is a subclass of Technology"){
        ...
        assertEquals(scl.isInstanceOf[Technology], true, "\"ProgrammingLanguage\" should be a subclass of \"Technology\"")
    }
